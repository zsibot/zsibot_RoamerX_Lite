...
ARGUMENTS_STATUS parse_arguments(
    const rclcpp::Logger& logger, int argc, char** argv, std::string& map_topic, SaveParameters& save_parameters)
{
                ...
                switch (commands[i].command_type)
                {
                    ...
                    case COMMAND_MODE:
                        ...
                        catch (std::invalid_argument&)
                        {
                            ...
                            RCLCPP_WARN(logger, "Map mode parameter not recognized: %s, using default value (trinary)", it->c_str());
                        }
                        break;
                }
                break;
