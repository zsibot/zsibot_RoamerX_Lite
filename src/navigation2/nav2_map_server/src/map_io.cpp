...
namespace nav2_map_server
{
    ...
    void loadMapFromFile(const LoadParameters& load_parameters, nav_msgs::msg::OccupancyGrid& map)
    {
        ...
        TIMER.end();
        RCLCPP_INFO(rclcpp::get_logger("map_io"), "Loading image time: %.9f", TIMER.elapsed_time_in_seconds());
        ...
        // --- 4) allocate data ---
        msg.data.assign(width * height, nav2_util::OCC_GRID_UNKNOWN);
...
#pragma omp parallel for schedule(static)
                ...
                for (size_t y = 0; y < height; ++y)
                {
                    ...
                    for (size_t x = 0; x < width; ++x)
                    {
                        ...
                        msg.data[base + x] = (occ > thrO)   ? nav2_util::OCC_GRID_OCCUPIED
                                             : (occ < thrF) ? nav2_util::OCC_GRID_FREE
                                                            : nav2_util::OCC_GRID_UNKNOWN;
                    }
                }
                break;
...
#pragma omp parallel for schedule(static)
                ...
                for (size_t y = 0; y < height; ++y)
                {
                    ...
                    for (size_t x = 0; x < width; ++x)
                    {
                        ...
                        {
                            cell = nav2_util::OCC_GRID_UNKNOWN;
                        }
                        else if (occ > thrO)
                        {
                            cell = nav2_util::OCC_GRID_OCCUPIED;
                        }
                        else if (occ < thrF)
                        {
                            cell = nav2_util::OCC_GRID_FREE;
                        }
                        else
                        {
...
#pragma omp parallel for schedule(static)
                ...
                for (size_t y = 0; y < height; ++y)
                {
                    ...
                    for (size_t x = 0; x < width; ++x)
                    {
                        ...
                        msg.data[base + x] = (shade >= nav2_util::OCC_GRID_FREE && shade <= nav2_util::OCC_GRID_OCCUPIED)
        ...
        TIMER.end();
        RCLCPP_INFO(rclcpp::get_logger("map_io"), "Construct map msg time: %.9f", TIMER.elapsed_time_in_seconds());
